...
export function MemberCard({ member }: MemberCardProps) {
  ...
  if (isDiretor) {
    return (
      <div
        className="col-span-full flex flex-col items-center gap-4 rounded-2xl p-8 text-center text-white sm:flex-row sm:text-left"
      ...
      >
        ...
        {member.avatarUrl ? (
          <img
            ...
            alt={member.name}
            className="h-20 w-20 flex-shrink-0 rounded-full border-2 border-white/20 object-cover"
          />
        ) : (
          <div className="flex h-20 w-20 flex-shrink-0 items-center justify-center rounded-full border-2 border-white/20 bg-white/10 text-xl font-bold text-white">
            {member.name
              .split(" ")
              .slice(0, 2)
              .map((n) => n[0])
  ...
  return (
    <div className="flex items-start gap-4 rounded-xl border border-gray-100 bg-white p-5 transition-all hover:border-gray-200 hover:shadow-sm">
      {/* Avatar */}
      {member.avatarUrl ? (
        <img
          src={member.avatarUrl}
